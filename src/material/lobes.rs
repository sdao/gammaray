...
impl Lobe for DisneyDiffuseRefl {
    fn f(&self, i: &core::Vec, o: &core::Vec) -> core::Vec {
        let f_in = util::schlick_weight(i.abs_cos_theta());
        let f_out = util::schlick_weight(o.abs_cos_theta());
...
impl DisneySpecularRefl {
    pub fn new(roughness: f32) -> DisneySpecularRefl {
        DisneySpecularRefl::new_aniso(roughness, 0.0)
    }

    pub fn new_aniso(roughness: f32, anisotropic: f32) -> DisneySpecularRefl {
...
impl Lobe for DisneySpecularRefl {
    fn f(&self, i: &core::Vec, o: &core::Vec) -> core::Vec {
        ...
        // XXX: wrong (no fresnel); simplified; also 0.2 instead of actual color.
        let half = half_unnorm.normalized();
        &core::Vec::one() * (0.2 * self.microfacet.d(&half) * self.microfacet.g(i, o) / (4.0 * cos_theta_out * cos_theta_in))
    }
